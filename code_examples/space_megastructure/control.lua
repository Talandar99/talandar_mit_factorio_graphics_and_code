...
local required_crafts = 5
local stage_names = {
	"megastructure-scaffolding",
	"megastructure-construction-stage-1",
	"megastructure-construction-stage-2",
	"megastructure-construction-stage-3",
	"megastructure",
}
...
for i = 1, 4 do
	local recipe_name = "megastructure-construct-" .. i
...
script.on_event(defines.events.on_script_trigger_effect, function(event)
	if event.effect_id == "megastructure-built" then
		local entity = event.target_entity or event.source_entity
		if not entity or not entity.valid then
			return
		end

		if entity.name == "megastructure-scaffolding" then
			storage.megastructure_progress = storage.megastructure_progress or {}
...
local function on_entity_removed(event)
	local entity = event.entity
	if entity.valid and entity.name:find("megastructure") then
		storage.megastructure_progress = storage.megastructure_progress or {}
		storage.megastructure_text_ids = storage.megastructure_text_ids or {}
